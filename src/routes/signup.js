...
const SignupComponent = () => {
    ...
    const signUp = async () => {
        // Frontend validation
        if (!email || !confirmEmail || !username || !password || !firstName || !lastName) {
            toast.error("All fields are required!");
            return;
        }
        if (email !== confirmEmail) {
            toast.error("Email and confirm email fields must match!");
            return;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        ...
        // Disable multiple submissions with a loading state
        setIsLoading(true);
        ...
        try {
            ...
            if (response.error) {
                toast.error(response.error); // Show error from backend (e.g., "Email already in use")
            } else {
            ...
            <div className="inputRegion w-1/3 py-10 flex items-center justify-center flex-col">
                ...
                <div className=" w-full flex items-center justify-center my-8">
                    <button
                        ...
                        className={`font-semibold p-3 px-10 rounded-full ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                        onClick={(e) => {
                            e.preventDefault();
                            if (!isLoading) signUp();
                        }}
